...
async def download_images_for_model(model_id, timeout_value):
    ...
    base_page_url = f"{base_url}?modelId={str(model_id)}"
    failed_urls = []
    images_without_meta = 0

    for page in range(1, 101): # Incremental pagination from page 1 to 100
        url = f"{base_page_url}&page={page}"  # Add the page parameter to the URL
        async with httpx.AsyncClient() as client:
            try:
                response = await client.get(url, timeout=timeout_value)
                if response.status_code == 200:
                    ...
                    print("Too many requests. Stopping the download process.")
                    break  # Abort at status code 429
                else:
...
async def download_images_for_username(username, timeout_value):
    global NEW_IMAGES_DOWNLOADED
    base_page_url = f"{base_url}?username={username.strip()}"
    failed_urls = []
    images_without_meta = 0
